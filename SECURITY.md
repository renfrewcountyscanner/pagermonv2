...
Do not open a public issue for a suspected vulnerability. Use GitHub's **Security** tab and select **Report a vulnerability**. If private vulnerability reporting is unavailable, contact the repository owner privately through GitHub.
...
- **Example configuration** contains placeholders only; `.env`, runtime configuration, certificates, and persistent data are ignored by Git

## Best Practices for Deployment

1. Always run `./install.sh` for a new Docker deployment — it generates random secrets
...
8. Do not commit `.env`, runtime configuration, database files, backups, or screenshots containing live traffic
